...
// Copied from rpiv-ask-user-question view/components/inline-input.ts (locales
// and rpiv specifics cut). Grapheme-aware extraction at the cursor: pi-tui's
// Editor reports UTF-16 line/column positions, so the cursor can land between
// code units of one cluster (emoji, ZWJ, combining marks). Single-code-unit
// slicing would split the cluster across the SGR 7/27 boundary.
...
export interface RenderInlineInputOptions {
	...
	contentWidth: number;
	/** Per-line styling (single-select: `theme.selectedText`). */
...
/**
 * Build the cursor-marked raw string for the whole buffer: `before |
 * CURSOR_MARKER | SGR-7 reverse-video cell | SGR-27 | after`. The cell UNDER
 * the cursor is the single grapheme at the offset (or U+00A0 NBSP at
 * end-of-buffer / on a literal space — NBSP is wrap-safe where a literal space
 * would tokenize as a wrap break). Zero characters shift; the column under the
 * cursor inverts. `CURSOR_MARKER` is zero-width so wrap/truncate math is
 * preserved.
 */
function buildCursorRaw(buffer: string, offset: number): string {
	...
	// A logical newline has no visible cell. Draw the cursor on an NBSP
	// immediately before it and leave the newline unconsumed so the next
	// logical line still renders.
	const cursorAtLineEnd = rawAt === "\n";
	const atCursor =
		rawAt === "" || rawAt === " " || cursorAtLineEnd ? "\xa0" : rawAt;
 ...
 * Cursor visualization follows Pi's editor pattern: reverse-video on the cell
 * at the cursor, with a non-breaking-space cell at end-of-line/end-of-buffer.
 */
