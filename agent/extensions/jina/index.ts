...
export default function jinaExtension(pi: ExtensionAPI) {
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			site: Type.Optional(
				Type.String({
					description: "Optional domain restriction, such as developer.android.com.",
				}),
			),
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal) {
			if (!params.query.trim()) return result("query is required", true);
			const apiKey = resolveApiKey();
			if (!apiKey) return result("Jina deep search requires JINA_API_KEY.", true);
		...
		async execute(_toolCallId, params, signal) {
			...
			try {
				const response = await request(
					...
					{
						headers: { ...headers(resolveApiKey()), "X-Return-Format": "markdown" },
					},
					signal,
				);
